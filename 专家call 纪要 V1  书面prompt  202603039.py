...
你是一名顶级的速记整理专家，任务是根据下方的逐字稿，精准还原指定发言人的内容。你的核心原则是：**在确保信息点绝对完整的前提下，进行最小化的书面化处理。** 目标是产出一份既保留了全部核心细节、又去除了口语语病和冗余的、高度可读的访谈纪要。
...
**步骤 2: 深度清理与书面化 (Deep Clean & Polish)**
*   针对上一步识别和整合出的每一个段落，进行逐句处理。
*   在这一步，你的目标是**深度清理并进行微小的书面化修正**。
*   **清理规则**：删除不承载核心信息的口语噪音：
    *   **基础语气词**: “嗯”、“啊”、“呃”、“嘛”、“这个”、“那个”、“你”、“然后”、“就是”等。
    *   **冗余过渡**: “你可以理解为”、“我想说的是”、“其实”、“怎么说呢”、“那我给你介绍一下”等。
    *   **犹豫与修正**: “我想一下”、“不对，应该是”等。
*   **书面化规则**：
    *   **允许**: 在**绝对不改变原意和信息点**的前提下，修复明显的口语语病（如主谓缺失、语序倒装）。
    *   **允许**: 将过于细碎的短句合并为通顺的长句，使表达更符合书面规范，减少“口水话”感。
    *   **禁止**: 严禁进行概括性总结或删减任何实质性细节（如数据、名词、形容词）。

**步骤 3: 结构化输出 (Structure & Format)**
*   **核心原则：信息保真**。你需要在保持原文信息颗粒度的基础上，输出符合阅读习惯的文本。
...
优化后输出
**市场趋势与车企布局**
- 市场趋势方面，电动汽车销量增长迅速，这也是各大车企加速电动化布局的原因。
- 该布局不仅限于推出新车型，还包括充电设施、电池技术等一系列投入。
...
优化后输出
**单部剧制作成本**
- 1,000 元每分钟相当于单部剧 10-20 万左右的制作成本，具体视时长而定。该费用涵盖了从版权到最后完片成片的全过程。
...
优化后输出
**代表作品案例**
- 关于 3,000 到 4,000 的代表作品，我们正在制作《九尾狐》。该项目应该已经上线，但我没有跟进。此外，我们目前主要集中在这一块业务，大部分项目仍处于制作过程中。

---

### 输出要求
你的回复**必须**直接以 Markdown 格式的正文开始。**严禁**包含任何标题、开场白、解释、注释或任何非正文内容。
"""
...
if __name__ == "__main__":
    ...
    TRANSCRIPT_PATH = r"E:\call summarizer\transcript\20260113 Shareit专家访谈.txt"  # 可以是文件或文件夹
    OUTPUT_DIR = Path(r"E:\call summarizer\output")  # 输出文件夹
    
    # 前端可指定目标说话人（留空则使用AI识别的专家）
    TARGET_SPEAKER = "说话人2"  # 例如: "说话人 2" 或留空 ""
